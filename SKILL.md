...
A skill for creating new skills and iteratively improving them — with deep awareness of the platform tools available in your environment.
...
## Tool Discovery — Know Your Environment

Before writing any skill that interacts with platform tools, you MUST discover what tools are actually available in the current session. This is critical because tools change — new MCP servers get added, tokens get configured or removed, and different environments have different capabilities.
...
Skills should NOT hardcode tool lists. Instead, they should:
...
#### Writing Skills That Use Platform Tools

When a skill relies on external tools (MCP servers, SQL, APIs), follow these patterns:
...
5. **Don't assume tool signatures**: Tool parameters change across versions. The skill should describe what data to pass, and the model will figure out the exact parameters at runtime by checking tool schemas.
...
5. **For tool-dependent skills, check tool usage patterns.** Did the model discover the right tools? Did it use them efficiently? If every test run starts with 5 minutes of tool discovery, consider adding a "prerequisite tools" section to the skill that names the tool categories needed. If the model is calling tools in a suboptimal order (e.g., querying before understanding the schema), restructure the skill's workflow to guide it.
...
- **Discover what tools are available** for the skill's domain
